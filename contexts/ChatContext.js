...
export function ChatProvider({ children }) {
    ...
    const buildSystemPrompt = useCallback(() => {
        ...
        prompt += `\n\nWhen the user wants to buy something on Amazon, follow this process for EACH item: 1) Use web_search to search for the specific product on Amazon (e.g. "amazon.com queen pillow case budget"). 2) Find the product URL in the results — look for the /dp/XXXXXXXXXX pattern where XXXXXXXXXX is the 10-character ASIN. 3) Once you have all ASINs, call amazon_cart with items containing each name and ASIN. IMPORTANT: Search for each product SEPARATELY to get accurate ASINs. Do NOT guess ASINs.`;
