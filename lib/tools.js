...
export const TOOLS = [
    ...
    },
    {
        name: 'amazon_cart',
        description:
            'Generate an Amazon "Add to Cart" link with pre-populated items. Use this after finding product ASINs (via web_search on amazon.com) when the user wants to buy something. Returns a clickable link that opens Amazon with items already in the cart.',
        input_schema: {
            type: 'object',
            properties: {
                items: {
                    ...
                    items: {
                        ...
                        properties: {
                            asin: {
                                type: 'string',
                                description: 'Amazon ASIN (10-character product identifier, e.g. "B0XXXXXXXXX").',
                            },
                            quantity: {
                                type: 'integer',
    ...
    switch (toolName) {
        ...
        case 'amazon_cart': {
            ...
            if (!items || items.length === 0) {
                return 'At least one item with an ASIN is required.';
            }

            // Validate ASINs (10-character alphanumeric)
            for (const item of items) {
                if (!item.asin || !/^[A-Z0-9]{10}$/i.test(item.asin)) {
                    return `Invalid ASIN: "${item.asin}". ASINs must be exactly 10 alphanumeric characters.`;
                }
            }
            ...
            const summary = items.map(item => {
                const label = item.label || item.asin;
                const qty = item.quantity || 1;
                return `${label} (x${qty})`;
            }).join(', ');

            return JSON.stringify({
                message: `🛒 Amazon cart ready with ${items.length} item${items.length > 1 ? 's' : ''}: ${summary}`,
                __amazon_cart: { url, items: items.map(i => ({ asin: i.asin, quantity: i.quantity || 1, label: i.label || i.asin })) },
            });
        }
