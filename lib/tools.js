...
export const TOOLS = [
    ...
    },
    {
        name: 'amazon_cart',
        description:
            'Generate Amazon shopping links for items the user wants to buy. Each item needs a search term (product name) and optionally an ASIN if you found one. The search link always works; the ASIN cart link is a bonus.',
        input_schema: {
            type: 'object',
            properties: {
                items: {
                    ...
                    items: {
                        ...
                        properties: {
                            name: {
                                type: 'string',
                                description: 'Product name / search query (e.g. "yoga mat extra thick").',
                            },
                            asin: {
                                type: 'string',
                                description: 'Optional Amazon ASIN if you found one. Must be exactly 10 alphanumeric characters.',
                            },
                            quantity: {
                                type: 'integer',
    ...
    switch (toolName) {
        ...
        case 'amazon_cart': {
            ...
            if (!items || items.length === 0) {
                return 'At least one item with a product name is required.';
            }

            const tag = process.env.AMAZON_ASSOCIATE_TAG;
            const tagParam = tag ? `&tag=${tag}` : '';

            // Build per-item Amazon search links (always reliable)
            const processedItems = items.map(item => {
                const searchUrl = `https://www.amazon.com/s?k=${encodeURIComponent(item.name)}${tagParam}`;
                const result = {
                    name: item.name,
                    quantity: item.quantity || 1,
                    searchUrl,
                };
                // If ASIN provided, also build a direct product link
                if (item.asin && /^[A-Z0-9]{10}$/i.test(item.asin)) {
                    result.productUrl = `https://www.amazon.com/dp/${item.asin}${tagParam ? '?' + tagParam.slice(1) : ''}`;
                }
                return result;
            });

            // Primary link: if multiple items, use search for first item; if single, use product or search
            const primaryUrl = processedItems.length === 1 && processedItems[0].productUrl
                ? processedItems[0].productUrl
                : processedItems[0].searchUrl;

            const summary = processedItems.map(i => `${i.name} (x${i.quantity})`).join(', ');

            return JSON.stringify({
                message: `🛒 Amazon link ready: ${summary}`,
                __amazon_cart: { url: primaryUrl, items: processedItems },
            });
        }
