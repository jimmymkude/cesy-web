...
export const TOOLS = [
    ...
    {
        name: 'save_group_memory',
        description:
            'Save a shared memory/note for a group. ONLY use this for things that are genuinely relevant to the ENTIRE group and were discussed IN the group chat (e.g. group goals, shared plans, group decisions, inside jokes between members). NEVER save personal topics, individual interests, or anything from private 1:1 conversations as a group memory — use save_memory for those instead. Group memories are visible to ALL group members.',
        input_schema: {
            type: 'object',
            properties: {
                groupId: {
                    type: 'string',
                    description: 'The group ID to save the memory to (from your context).',
                },
                content: {
                    type: 'string',
                    description: 'The memory content to save for the group.',
                },
                tags: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'Tags to categorize the memory, e.g. ["goal", "plan"].',
                },
            },
    ...
    {
        name: 'search_group_memories',
        description:
            'Search shared memories of a group. Use this to recall group-level context like goals, plans, decisions, or inside jokes. Only works if the user is a member of the group.',
        input_schema: {
            type: 'object',
            properties: {
                groupId: {
                    type: 'string',
                    description: 'The group ID to search memories in.',
                },
                query: {
                    type: 'string',
                    description: 'Search query — a keyword or phrase to find relevant group memories.',
                },
            },
...
export async function executeTool(toolName, toolInput, userId) {
    switch (toolName) {
        ...
        // ── Group Memories ───────────────────────────────────────
        case 'save_group_memory': {
            const { groupId, content, tags = [] } = toolInput;

            // Verify membership
            const membership = await prisma.groupMember.findFirst({
                where: { groupId, userId },
            });
            if (!membership) return 'You are not a member of this group.';

            // Deduplicate
            const existing = await prisma.groupMemory.findFirst({
                where: { groupId, content },
            });
            if (existing) return 'Group memory already saved.';

            const mem = await prisma.groupMemory.create({
                data: { groupId, content, tags },
            });

            // Generate and store embedding
            const emb = await generateEmbedding(content);
            if (emb) {
                const vecStr = toVectorLiteral(emb);
                await prisma.$executeRawUnsafe(
                    `UPDATE group_memories SET embedding = $1::vector WHERE id = $2`,
                    vecStr,
                    mem.id
                );
            }

            return `Saved group memory: "${content}"`;
        }

        case 'search_group_memories': {
            const { groupId, query } = toolInput;
            ...
            if (qEmb) {
                const vecStr = toVectorLiteral(qEmb);
                const results = await prisma.$queryRawUnsafe(
                    `SELECT id, content, created_at as "createdAt"
                     FROM group_memories
                ...
                if (results.length === 0) return 'No group memories found for this query.';
                return results
                    .map((m) => `- ${m.content} (${new Date(m.createdAt).toLocaleDateString()})`)
                    .join('\n');
            }
            ...
            const mems = await prisma.groupMemory.findMany({
                where: {
                    groupId,
                    content: { contains: query, mode: 'insensitive' },
                },
            ...
            if (mems.length === 0) return 'No group memories found for this query.';
            return mems
                .map((m) => `- ${m.content} (${new Date(m.createdAt).toLocaleDateString()})`)
                .join('\n');
        }
