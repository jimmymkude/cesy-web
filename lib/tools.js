    ...
    switch (toolName) {
        ...
        case 'cancel_reminder': {
            ...
            if (reminders.length === 0) {
                const all = await prisma.reminder.findMany({ where: { userId, completed: false }, orderBy: { dueAt: 'asc' }, take: 10 });
                if (all.length === 0) return 'No active reminders found.';
                const list = all.map(r => `- "${r.content}" (due ${new Date(r.dueAt).toLocaleString()})`).join('\n');
                return `No reminders matching "${query}". Active reminders:\n${list}\nRetry with a keyword from the list above.`;
            }
        ...
        case 'update_reminder': {
            ...
            if (Object.keys(updateData).length === 0) {
                return `No changes provided for reminder: "${target.content}".`;
            }
            ...
            return `Updated reminder "${target.content}": ${changes.join(', ')}.`;
        }
