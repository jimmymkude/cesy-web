        ...
        case 'amazon_cart': {
            ...
            const processedItems = items.map(item => {
                ...
                if (item.asin && /^[A-Z0-9]{10}$/i.test(item.asin)) {
                    // ASIN cart URL — adds directly to Amazon cart
                    result.url = `https://www.amazon.com/gp/aws/cart/add.html?ASIN.1=${item.asin}&Quantity.1=${result.quantity}${tagParam}`;
                    result.asin = item.asin;
                } else {
                    // Fallback to search
                    result.url = `https://www.amazon.com/s?k=${encodeURIComponent(item.name)}${tag ? `&tag=${tag}` : ''}`;
                }
                return result;
            });
            ...
            return JSON.stringify({
                message: `🛒 Amazon cart links ready: ${summary}`,
