...
export const TOOLS = [
    ...
    },
    {
        name: 'get_calendar',
        description:
            'Get the user\'s upcoming reminders and workout schedule for a given date or today. Use this when the user asks about their schedule, what\'s coming up, or what they have planned.',
        input_schema: {
            type: 'object',
            properties: {
                date: {
...
export async function executeTool(toolName, toolInput, userId) {
    switch (toolName) {
        ...
        case 'send_notification': {
            ...
            // Stub implementation — logs intent, returns confirmation
            // Future: wire to WhatsApp server or browser push API
            console.log(`[Notification] Channel: ${channel} | Message: ${message} | User: ${userId}`);
            return `Notification queued via ${channel}: "${message}"`;
        }
        ...
        case 'set_timer': {
            ...
            return `Timer started: "${label}" — ${display}`;
        }
